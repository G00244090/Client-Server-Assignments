...
public class MapSample extends CollectionSample implements PropertyChangeListener {
	...
	private WeakHashMap<Integer, Integer> WeakHash;
	private ConcurrentHashMap<Integer, Integer> ConcurrentHash;
	private IdentityHashMap<Integer, Integer> Identity;
	private ConcurrentSkipListMap<Integer, Integer> ConcurrentSkipList;
	private Task task;
	...
	public MapSample(StatsPanel statsPanel, Main main) {
		...
		WeakHash = new WeakHashMap<Integer, Integer>();
		ConcurrentHash = new ConcurrentHashMap<Integer, Integer>();
		Identity = new IdentityHashMap<Integer, Integer>();
		ConcurrentSkipList = new ConcurrentSkipListMap<Integer, Integer>();
	}
	...
	private class Task extends SwingWorker<Void, Void> {
		...
		private long[] fillMaps() {
			...
			for (int z = 0; z <= iterations; z++) {
				...
				for (int j = 0; j < getListSize(); j++) {
					ConcurrentHash.put(keys[j], values[j]);
				}
				times[0] += System.nanoTime() - startingTime;

				startingTime = System.nanoTime();
				for (int j = 0; j < getListSize(); j++) {
					Identity.put(keys[j], values[j]);
				}
				times[1] += System.nanoTime() - startingTime;

				startingTime = System.nanoTime();
				for (int j = 0; j < getListSize(); j++) {
					ConcurrentSkipList.put(keys[j], values[j]);
				}
				times[2] += System.nanoTime() - startingTime;

				startingTime = System.nanoTime();
				for (int j = 0; j < getListSize(); j++) {
					WeakHash.put(keys[j], values[j]);
				}
		...
		private long[] removeFromMaps() {
			...
			for (int z = 0; z <= iterations; z++) {
				...
				for (int j = 0; j < 50; j++)
					ConcurrentHash.remove(toBeRemoved[j]);
				times[0] += System.nanoTime() - startingTime;

				startingTime = System.nanoTime();
				for (int j = 0; j < 50; j++)
					Identity.remove(toBeRemoved[j]);
				times[1] += System.nanoTime() - startingTime;

				startingTime = System.nanoTime();
				for (int j = 0; j < 50; j++)
					ConcurrentSkipList.remove(toBeRemoved[j]);
				times[2] += System.nanoTime() - startingTime;

				startingTime = System.nanoTime();
				for (int j = 0; j < 50; j++)
					WeakHash.remove(toBeRemoved[j]);
				times[3] += System.nanoTime() - startingTime;
		...
		private long[] iterateOnMaps() {
			...
			for (int z = 0; z <= iterations; z++) {
				...
				long startingTime = System.nanoTime();
				Iterator<Integer> iterator = ConcurrentHash.keySet().iterator();
				...
				startingTime = System.nanoTime();
				iterator = Identity.keySet().iterator();
				while (iterator.hasNext()) {
					iterator.next();
				}
				times[1] += System.nanoTime() - startingTime;
				...
				startingTime = System.nanoTime();
				iterator = ConcurrentSkipList.keySet().iterator();
				while (iterator.hasNext()) {
					iterator.next();
				}
				times[2] += System.nanoTime() - startingTime;
				...
				startingTime = System.nanoTime();
				iterator = WeakHash.keySet().iterator();
				while (iterator.hasNext()) {
					iterator.next();
				}
				times[3] += System.nanoTime() - startingTime;
